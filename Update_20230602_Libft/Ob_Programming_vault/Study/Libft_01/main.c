...
void ft_test_del_lstdelon(void *content) 
{
    free(content);
}

void	ft_test_lstiter(void *elem)
{
	int		len;
	char	*content;
...
void *ft_test_f_lstmap(void *content) 
{
  char *original_str = (char *)content;
  size_t len = strlen(original_str);
  char *new_str = (char *)malloc(len + 2); // +2 for the "!" and null terminator
  strcpy(new_str, original_str);
  strcat(new_str, "!");
  return new_str;
}

t_list *ft_creat_element(void)
{
  t_list		*elem;
	t_list		*elem2;
	t_list		*elem3;
...
void print_list(t_list *lst)
{
  ...
  while (lst_use != NULL )
  {
    printf("Data %lu: %s \n", count, (char *)lst_use->content);
    lst_use = lst_use->next;
    count++;
  }
}
...
*                     BONUS                               *
***********************************************************/
ft_printseperater_part("BONUS");
//ft_lstnew
/*
ft_printseperater("ft_lstnew");
char *content_ft_lstnew = "42 Berlin";
t_list *ret_ft_lstnew;
ret_ft_lstnew = ft_lstnew(content_ft_lstnew);
print_list(ret_ft_lstnew);
*/
//ft_lstadd_front
/*
  ft_printseperater("ft_lstadd_front");
  t_list *elem_lstadd_front = ft_creat_element();
  char *str_lstadd_front = "Test_lstadd_front";
  t_list *new_lstadd_front = ft_lstnew(str_lstadd_front);

  print_list(elem_lstadd_front);
  printf("\n-------------------------------\n");

  ft_lstadd_front(&elem_lstadd_front, new_lstadd_front);
  print_list(elem_lstadd_front);
*/
//ft_lastsize
/* 
  ft_printseperater("ft_lstsize");
  t_list *elem_lstsize = ft_creat_element();

  print_list(elem_lstsize);
  printf("\n-------------------------------\n");
  printf("\nAfter using function -> Result: |%d|\n", ft_lstsize(elem_lstsize));
*/
//ft_lstlast
/*
  ft_printseperater("ft_lstlast");
  t_list *elem_lstlast = ft_creat_element();
  t_list *ret_lstlast;
  
  print_list(elem_lstlast);
  printf("\n-------------------------------\n");
  ret_lstlast = ft_lstlast(elem_lstlast);
  printf("\nAfter using function -> Result: |%s|\n", (char *)ret_lstlast->content);
*/
//ft_lst_back
/*
  ft_printseperater("ft_lstadd_back");
  t_list *elem_lstadd_back = ft_creat_element();
  char *str_lstadd_back = "Test_lstadd_back";
  t_list *new_lstadd_back = ft_lstnew(str_lstadd_back);

  print_list(elem_lstadd_back);
  printf("\n-------------------------------\n");
  ft_lstadd_back(&elem_lstadd_back, new_lstadd_back);
  print_list(elem_lstadd_back);
*/
//ft_lstdelone
/*
  ft_printseperater("ft_lstdelone");
  t_list *elem_lstdelone = ft_creat_element();

  print_list(elem_lstdelone);
  printf("\n-------------------------------\n");
  ft_lstdelone(elem_lstdelone, &ft_test_del_lstdelon);
  printf("\nAfter using function\n");
  print_list(elem_lstdelone);
  //reason for core dumpe is the hole 
*/ 
//lst_clear
/*
  ft_printseperater("ft_lstclear");
  t_list *elem_lstclear = ft_creat_element();
  print_list(elem_lstclear);
  printf("\n-------------------------------\n");
  void (*ft_test_del_lstclear)(void *) = ft_test_del_lstdelon;
  ft_lstclear(&elem_lstclear, ft_test_del_lstclear);
  printf("\nAfter using function\n");
  print_list(elem_lstclear);
  if (!elem_lstclear)
  {
  ...
  {
    while (elem_lstclear != NULL)
      {
        elem_lstclear = elem_lstclear->next;
        printf("content: %s\n", (char *)elem_lstclear->content);
      }
  }
  */
//ft_lstiter
/*
  ft_printseperater("ft_lstiter");
  t_list *elem_lstiter = ft_creat_element();

  print_list(elem_lstiter);
  printf("\n-------------------------------\n");

  ft_lstiter(elem_lstiter, ft_test_lstiter);
  printf("\nAfter using function\n");
  print_list(elem_lstiter);
*/
//ft_lstmap;
  /*
  ft_printseperater("ft_lstmap");
  t_list *elem_lstmap = ft_creat_element();

  print_list(elem_lstmap);
  printf("\n-------------------------------\n");
  void (*ft_test_del_lstmap)(void *) = ft_test_del_lstdelon;
  t_list *new_list = ft_lstmap(elem_lstmap, ft_test_f_lstmap, ft_test_del_lstmap);
